import type { SaveData } from '../core/Save';

/**
 * お知らせ。
 *
 * 受信箱と分ける。あちらは「受け取る物がある」画面なので、
 * 読み物を混ぜると受け取り漏れが埋もれる。こちらは読むだけで、
 * 既読にする以外の操作を置かない。
 *
 * 新しいものが先頭。日付は表示のためだけに持つ——並びは配列の順で決める。
 */

export type NewsTag = 'update' | 'event' | 'notice';

export const NEWS_TAGS: Record<NewsTag, string> = {
  update: '更新',
  event: 'イベント',
  notice: 'お知らせ',
};

export interface NewsDef {
  id: string;
  date: string;
  tag: NewsTag;
  title: string;
  body: string;
}

export const NEWS: NewsDef[] = [
  {
    id: 'n-018', date: '09/25', tag: 'update',
    title: '編成が5体になりました',
    body: '出撃できる枠を3から5に増やしました。相手も同じ数で出てきます。'
      + '手持ちが5体に満たないうちは、持っているぶんだけで出撃し、相手もその数に揃います。'
      + '味方全体に効く特性は、配る相手が増えたぶん1体あたりを薄くしました'
      + '（共鳴：必殺 +10 → +6、掌握する空：攻撃 +5% → +3%、制海：必殺獲得 −15% → −12%）。'
      + 'あわせて全28種の数値を5対5で取り直し、巨獣の体力も5体で挑む前提に合わせています。',
  },
  {
    id: 'n-017', date: '09/25', tag: 'event',
    title: '今日の戦場・巨獣討伐がはじまりました',
    body: 'バトルの「イベント」に、日付で入れ替わる2つの戦いを加えました。'
      + '「今日の戦場」は毎日 4:00 に場の決まりが変わります。属性が通りやすくなる日、'
      + '壁ばかりが待ち構える日、足の速い者が雪崩れ込む日、誰もが焦る乱戦の日——'
      + '札に書かれた勝ち筋に合わせて編成を組むと、ぐっと勝ちやすくなります。'
      + 'その日の初勝利で、コインと化石が1つ手に入ります。'
      + '「巨獣討伐」は毎週月曜に相手が替わる、1体きりの巨獣との60秒勝負です。'
      + '与えたダメージが 25%・55% に届くと銅・銀の報酬、倒しきれば金の報酬として'
      + 'その巨獣と同じ種の化石が手に入ります。段ごとに週1回ずつ受け取れます。',
  },
  {
    id: 'n-016', date: '09/21', tag: 'update',
    title: '必殺技の発動と、行動順の不具合',
    body: '必殺技はゲージが満ちた時点で撃つようになりました。'
      + 'これまでは「敵が2体以上いる」「一撃で倒せる」「強気の構え」のいずれかを'
      + '満たしたときだけ撃っていたので、残り1体の詰めでゲージを満タンにしたまま'
      + '通常攻撃を繰り返すことがありました。設定の「必殺技」を手動にすると、'
      + 'これまで通りカードを押すまで溜めます。'
      + 'あわせて、相手のほうが多く行動しているように見える不具合を直しました。'
      + '新しく追加した7種の必殺技に技後の隙の設定が抜けており、撃った個体の行動順が'
      + '壊れて、以後その個体が一度も動かなくなっていました。'
      + 'この不具合を前提に調整していた数値も、全28種ぶん取り直しています。'
      + 'カードの攻撃間隔リングが、行動した直後に満ちて見える問題も直しました。'
      + '演出が1手ぶんの間隔より長くなると、そのぶん表示だけが先へ進んで'
      + '上限に張り付いていました。',
  },
  {
    id: 'n-015', date: '09/21', tag: 'event',
    title: '6種を追加しました',
    body: 'ディモルフォドン（風★1・打撃役）、オフタルモサウルス（水★2・技巧役）、'
      + 'アルベルタケラトプス（土★3・妨害役）、カルノタウルス（火★3・仕留め役）、'
      + 'テリジノサウルス（土★4・崩し役）、エラスモサウルス（水★4・特攻役）。'
      + 'あわせて「毒」を追加しました。火傷と違って重ねられ、1つにつき毎行動 最大体力の 4% を削ります（4つまで）。'
      + '攻撃力とは無関係なので、硬い相手ほど殴るより効きます。'
      + 'エラスモサウルスは通常攻撃でも必殺でも毒を重ねます。',
  },
  {
    id: 'n-014', date: '09/21', tag: 'event',
    title: '★5 ティラノサウルス スー',
    body: '無属性の頂点種を追加しました。特性「歴戦の暴君」は属性相性を使わず、'
      + '与えるダメージも受けるダメージも、相手の属性に関わらず ×1.5 になります。'
      + '有利も不利も無い代わりに、殴る側も殴られる側も倍率が乗ったままです——'
      + '硬い相手には押し負けます。必殺「覇王鎮魂・六千万年ノ咬」は単体に特大ダメージを与え、'
      + '2行動のあいだ対象の被ダメージを +20% にします。'
      + 'ティラノサウルスの特性「不変」だけがこの倍率を打ち消します。',
  },
  {
    id: 'n-013', date: '09/20', tag: 'update',
    title: 'ミッションと商店を開きました',
    body: '拠点にミッションの帯を追加しました。いま最もクリアに近い1件が出ます。'
      + '一覧からは日課と記録の両方を確認でき、達成したものはその場で受け取れます。'
      + 'あわせてアイテムショップを開きました。掘り出せなかった原石と調査許可証を、'
      + '発掘で貯めたコインと交換できます。',
  },
  {
    id: 'n-012', date: '09/20', tag: 'update',
    title: 'バトル中の動きを作り直しました',
    body: 'ユニットが持ち場に張り付かなくなりました。役割ごとに間合いと歩調が違い、'
      + '壁役は前に出たまま構え、脚の速い役ほど広く動き回り、支援役は下がって間合いを取ります。'
      + '攻撃はその場の踏み込みではなく、相手まで歩いて戻る動きになりました。'
      + '勝敗の計算には手を入れていないので、同じ編成の強さは変わりません。',
  },
  {
    id: 'n-011', date: '09/19', tag: 'event',
    title: '★5 ハツェゴプテリクス / ブラキオサウルス',
    body: '2種を追加しました。ハツェゴプテリクスは特性「島の頂点」で、'
      + '攻撃に成功したとき一度だけ攻撃力が永続で上がります。'
      + 'ブラキオサウルスは特性「大地の伊吹」で味方の回復量を底上げし、'
      + '必殺「磨り潰し消化」で味方全体を継続回復させます。',
  },
  {
    id: 'n-010', date: '09/19', tag: 'update',
    title: '精錬の採点を変えました',
    body: '損傷は別枠で引くのではなく、得点の上限を下げる形にしました。'
      + '骨を1つ傷つけるごとに上限が 3 点下がります。丁寧に削れば上限のまま、'
      + '荒く削れば上限そのものが落ちる——どこで損をしたかが読める採点になります。'
      + '岩と化石の色の差も広げ、真ん中の抜けも直しました。',
  },
  {
    id: 'n-009', date: '09/18', tag: 'notice',
    title: '図鑑を未所持でも開けるようにしました',
    body: '未所持の個体も姿と性能を確かめられます。'
      + '属性・レア度・役割・所持で並べ替えができ、選んだ軸をもう一度押すと昇降が入れ替わります。',
  },
  {
    id: 'n-008', date: '09/17', tag: 'notice',
    title: '受信箱を開きました',
    body: 'ログインボーナスと運営からの配布が受信箱に届きます。'
      + '受け取り済みの便りも残るので、何をいつ受け取ったかを後から確かめられます。',
  },
];

export function unreadNews(d: SaveData): number {
  return NEWS.filter((n) => !d.news.read.includes(n.id)).length;
}

export function isUnread(d: SaveData, id: string): boolean {
  return !d.news.read.includes(id);
}

/** 開いたぶんをまとめて既読にする。読んだ数を返す */
export function markAllRead(d: SaveData): number {
  const fresh = NEWS.filter((n) => !d.news.read.includes(n.id));
  d.news.read.push(...fresh.map((n) => n.id));
  return fresh.length;
}
